...
K_N = -1/R*z_N*alpha - 1/R/R/h*z_N_m12*kappa_N_m12 - cons*(k_N_m12*y_N_m12**3/2 + k_N*y_N**3*z_N)
...
def get_D(z, ys):
    D = [0] * (N + 1)
    h = z[1] - z[0]
    D[0] = z[0] * F0 / R + np ** 2 * sigma * h * T0 ** 4 * (k_t(ys[0]) * z[0] + (k_t(ys[0]) + k_t(ys[1])) / 2 * (z[0] + z[1]) / 2)
    for n in range(1, N):
        z_n_m_half = (z[n - 1] + z[n]) / 2
        z_n_p_half = (z[n + 1] + z[n]) / 2
...
run = True
T_sp = progonca(z_sp,T_sp)
cnt = 1
while run:
    cnt+=1
    tmp = progonca(z_sp,T_sp)
