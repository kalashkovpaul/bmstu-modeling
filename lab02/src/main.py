...
def runge_kutta_first_order(xn, yn, zn, hn, Rp):
    y_next = yn + hn * f(xn, yn, zn, Rp)
    z_next = zn + hn * phi(xn, yn, zn)
...
def runge_kutta_second_order(xn, yn, zn, hn, Rp):
    beta = 0.5
    y_next = yn + hn * ((1 - beta) * f(xn, yn, zn, Rp) + beta * f(xn + hn / (2 * beta),
                yn + hn / (2 * beta) * f(xn, yn, zn, Rp),
                zn + hn / (2 * beta) * phi(xn, yn, zn), Rp))

    z_next = zn + hn * ((1 - beta) * phi(xn, yn, zn) + beta * phi(xn + hn / (2 * beta),
                yn + hn / (2 * beta) * f(xn, yn, zn, Rp),
                zn + hn / (2 * beta) * phi(xn, yn, zn)))
...
if __name__ == "__main__":
    ...
    I_1order = I_2order = I_4order = I0
    Uc_1order = Uc_2order = Uc_4order = Uc0
    ...
    h = 1e-6
    for t in arange(0, 0.0006, h):
        Rp_1order = Rp(I_1order)
        Rp_2order = Rp(I_2order)
        Rp_4order = Rp(I_4order)
        if t > h:
            t_plot.append(t)
            I_1order_plot.append(I_1order)
            T_1order_plot.append(T0)
            U_1order_plot.append(Uc_1order)
            Rp_1order_plot.append(Rp_1order)
        ...
        I_1order, Uc_1order = runge_kutta_first_order(t, I_1order, Uc_1order, h, Rp_1order)
    ...
    plt.figure(3)
    plt.subplot(321)
    plt.plot(t_plot, I_1order_plot, label='I (1 order)')
    plt.xlabel('t')
    plt.ylabel('I, А (1 order)')
    plt.grid(True)

    plt.subplot(322)
    plt.plot(t_plot, U_2order_plot, label='Uc (1 order)')
    plt.xlabel('t')
    plt.ylabel('Uc, В (1 order)')
    plt.grid(True)

    plt.subplot(323)
    plt.plot(t_plot, Rp_1order_plot, label='Rp (1 order)')
    plt.xlabel('t')
    plt.ylabel('Rp, Ом (1 order)')
    plt.grid(True)

    for i in range(len(I_1order_plot)):
        I_1order_plot[i] *= Rp_1order_plot[i]

    plt.subplot(324)
    plt.plot(t_plot, I_1order_plot, label='I*Rp (1 order)')
    plt.xlabel('t')
    plt.ylabel('I*Rp, В (2 order)')
    plt.grid(True)

    plt.subplot(325)
    plt.plot(t_plot, T_1order_plot, label='T (1 order)')
    plt.xlabel('t')
    plt.ylabel('T, К (1 order)')
    plt.grid(True)
